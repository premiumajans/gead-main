...
//             var $el = $(this), content = $el.html(), qty = $el.attr('data-clone-element');
//             for (var i = 0; i < qty; i++) {
...
//         if ($(this).data('type')) {
//             $('.thumb-info-floating-caption-title', $(this)).append('<div class="thumb-info-floating-caption-type">' + $(this).data('type') + '</div>').css({'padding-bottom': 22});
//         }
...
(function (theme, $) {
    ...
    PluginAnimatedContent.prototype = {
        initialize: function ($el, opts) {
            ...
            this.options = $.extend(true, {}, PluginAnimatedContent.defaults, opts, {wrapper: this.$el});
            return this;
        }, build: function () {
            var self = this;
            ...
            if (self.options.contentType == 'letter') {
                ...
                var words = self.$el.text().split(" "), delay = self.options.startDelay;
                self.$el.empty();
        ...
        initialize: function ($el, opts) {
            ...
            if ($el.visible(true)) {
                if (!self.options.horizontal) {
                    ...
                    $el.css({transform: 'translate3d(' + minus + scrollPercent / self.options.speed + '%, 0, 0)'});
                }
            }
        }
    };
...
(function (theme, $) {
    ...
    PluginHoverEffect.prototype = {
        initialize: function ($el, opts) {
            ...
            this.options = $.extend(true, {}, PluginHoverEffect.defaults, opts, {wrapper: this.$el});
            return this;
        }, build: function () {
            var self = this;
            ...
            if ($.isFunction($.fn['hover3d'])) {
                self.$el.hover3d({selector: self.options.selector, sensitivity: self.options.sensitivity});
            }
            return this;
        }
    };
...
(function (theme, $) {
    ...
    PluginInViewportStyle.prototype = {
        initialize: function ($el, opts) {
            ...
            var self = this, el = self.$el.get(0);
            ...
            var parallaxEffectOnScrolResize = function () {
                $window.on('scroll resize', function () {
                    ...
                    if (!self.options.parallaxScale) {
                        ...
                        var scrollTop = $window.scrollTop(), elementOffset = self.options.wrapper.offset().top,
                            currentElementOffset = (elementOffset - scrollTop),
                            scrollPercent = Math.abs(+(currentElementOffset - $window.height()) / (self.options.startOffset ? self.options.startOffset : 7));
                        scrollPercent = parseInt((scrollPercent >= 100) ? 100 : scrollPercent);
                        var currentScale = (scrollPercent / 100) * 50;
                        if (!self.options.parallaxScaleInvert) {
                            background.css({'transform': 'scale(1.' + String(currentScale).padStart(2, '0') + ', 1.' + String(currentScale).padStart(2, '0') + ')'});
                        } else {
                            background.css({'transform': 'scale(1.' + String(50 - currentScale).padStart(2, '0') + ', 1.' + String(50 - currentScale).padStart(2, '0') + ')'});
                        }
                    }
                });
...
(function (theme, $) {
    ...
    $.extend(theme, {
        PluginValidation: {
            defaults: {
                formClass: 'needs-validation', validator: {
    ...
    $.extend(theme, {
        Nav: {
            ...
            events: function () {
                ...
                //                 force = ($this.is("[data-hash-force]") ? true : false), windowWidth = $(window).width();
            ...
            scrollToTarget: function (target, offset) {
                var self = this, targetPosition = $(target).offset().top;
                $('body').addClass('scrolling');
                $('html, body').animate({scrollTop: $(target).offset().top - offset}, self.options.scrollDelay, self.options.scrollAnimation, function () {
                    ...
                    if ($(target).offset().top != targetPosition) {
                        $('html, body').animate({scrollTop: $(target).offset().top - offset}, 1, self.options.scrollAnimation, function () {
                        });
                    }
                });
                return this;
